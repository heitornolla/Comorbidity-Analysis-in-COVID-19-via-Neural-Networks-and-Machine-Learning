...
def train(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader) -> nn.Module:
    ...
    for epoch in range(epochs):
        ...
        if epoch % 20 == 0:
            print(f'Epoch {epoch}: Val Loss = {avg_val_loss:.4f} | Acc = {acc:.4f} | Recall = {recall:.4f}')
        
    model.load_state_dict(torch.load(f'ckpt/{str(model)}_best_val.pth', weights_only=False))
    return model
...
if __name__ == '__main__':
    ...
    for model in models:
        ...
        evaluate_model(targets, preds, model_name=str(model()), log_file=f'logs/{str(model())}.csv')
